...
const ShortManagePage: React.FC = () => {
  ...
  const { getShortByStatus, shortByStatus, updateShortStatus } = useShort();
  const [selectedContentId, setSelectedContentId] = useState<number>(0);
  const [openIsOpenDialog, setIsOpenDialog] = useState(false);
  ...
  const total = shortByStatus?.total || 0;
  const totalPages = Math.ceil(total / pageSize);
  const shorts = shortByStatus?.content || [];

  const fetchShorts = async () => {
    setLoading(true);
    try {
      await getShortByStatus(status, currentPage, pageSize, debouncedSearch);
    } catch (error) {
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (status === "ACTIVE") {
            updateShortStatus(selectedContentId, "BANNED", () => {
              getShortByStatus(status, currentPage, pageSize, debouncedSearch);
            });
          } else {
            updateShortStatus(selectedContentId, "ACTIVE", () => {
              getShortByStatus(status, currentPage, pageSize, debouncedSearch);
            });
          }
        }}
        open={openIsOpenDialog}
      />
    </>
  );
};

export default ShortManagePage;
